...
```{r figure_s1, fig.height = 5, fig.width = 7.5}
###############################
# Figure S1
# V2 dose response
###############################
...
# load data
load("~/Dropbox/AndersenLab/LabFolders/Katie/projects/eQTL_mediation/manuscript/data/FileS1_dosepruned.Rda")

# adjust for differences in control with subtraction
dosepruned_sub <- NULL
for(t in c("median.TOF", "median.norm.EXT", "norm.n")) {
...
dosepruned_sub %>%
    ...
    # ggplot2::geom_boxplot(alpha = 0.5, outlier.color = NA, size = 0.2) +
    ...
    ggplot2::labs(x = "Dose (µM)", y = "Relative animal length") +
    ggplot2::scale_color_manual(values = c("N2" = "orange", "CB4856" = "blue",
                                          "DL238" = "green", "JU258" = "purple"),
                               name = "Strain")

# ggsave("~/Dropbox/AndersenLab/LabFolders/Katie/projects/eQTL_mediation/manuscript/figures/supp/s1_dosev2.pdf", height = 5, width = 7.5)
...
load("~/Dropbox/AndersenLab/LabFolders/Katie/projects/eQTL_mediation/manuscript/data/FileS2_riailpheno.Rda")
...
plot_pxg <- function(cross, map, tsize = 12) {
    ...
    pheno <- cross$pheno %>% 
        tidyr::gather(trait, phenotype, -strain, -set) %>%
        dplyr::mutate(trait = stringr::str_replace(trait, "\\.", "_")) %>%
    ...
    geno %>%
        ...
        ggplot2::facet_wrap(trait~marker, nrow = nrow(peaks), scales = "free") + 
        ggplot2::theme_bw(tsize) + 
...
for(c in unique(peaks$condition)) {
c <- "amsacrine"
    conddf <- peaks %>%
...
```{r figure_s3, fig.height = 8, fig.width = 7.5}
###############################
# Figure S3
...
for(c in unique(nildose$condition)) {
    nildose2 <- nildose %>%
        dplyr::filter(trait == "median.EXT", condition == c) 
...
nildose_sub %>% 
    ...
    ggplot2::facet_wrap(~condition, scales = "free", nrow = 4) +
    ggplot2::labs(x = "Dose (µM)", y = "Relative animal length") +
...
**Figure S3** Dose response for NILs with modified HTA. Relative animal length (median.EXT, y-axis) measured for each strain, colored by genotype, across five drug concentrations (µM, x-axis). For each strain, phenotype is subtracted from the control. A red asterisk indicates the dose selected for further experiments.
...
```{r figure_s5,  fig.height = 6, fig.width = 7.5}
###############################
# Figure S5
...
# load data
load("~/Dropbox/AndersenLab/LabFolders/Katie/projects/eQTL_mediation/manuscript/data/FileS10_set1_mappingdf.Rda")
...
```{r figure_s6,  fig.height = 6, fig.width = 7.5}
###############################
# Figure S6
# scb-1 del results
###############################
...
**Figure S6** Causality testing for *scb-1*. Drug phenotypes are plotted as Tukey box plots with strain (x-axis) by relative residual animal length (median.EXT, y-axis). Strains with an N2 background are colored orange and strains with a CB4856 background are colored blue. NILs are colored grey. Statistical significance of each strain compared to its parental strain (ECA232, ECA1132, and ECA1135 to N2 and ECA1114, ECA1133, and ECA1134 to CB4856) is shown above each strain and colored by the parent strain it was tested against (ns = non-significant (p-value > 0.05); `*`, `**`, `***`, and `***` = significant (p-value < 0.05, 0.01, 0.001, or 0.0001, respectively).
